...
const mainTestInfo = getMainTestInfo();
...
for (const file of userTestInfo.files) {
    ...
    const existingFile = mainTestInfo.find(obj => obj.path === file.path);
...
function getMainTestInfo() {
    let mainTestInfo;
    fetch(`https://api.github.com/repos/${repository}/contents/.test/test-info.json`)
